...
(function() {
    ...
    window.TTS_Float = {
        ...
        playFromTop: function() {
            // Find the first paragraph or header currently visible in the viewport
            const elements = document.querySelectorAll('p, h2, h3, blockquote, li');
            ...
            if (foundElement) {
                // Collect all text starting from this element
                let collect = false;
                const contentNodes = document.querySelectorAll('p, h2, h3, blockquote, li');
                const textBuffer = [];

                contentNodes.forEach(node => {
                    if (node === foundElement) collect = true;
                    if (collect) textBuffer.push(node.innerText);
                });
